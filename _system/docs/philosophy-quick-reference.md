...
- **Efforts** (work): project, workstream, incident
- **Horizons** (object): pillar, mission, milestone
- **Planning** (object): inquiry, idea, decision
- **Utility** (object): trigger, skill
- **Actors** (object): person, user, agent, organization
- **Knowledge** (object): note, document, reference, quote, framework, pain-point, value-prop, audience, principle
- **Artifacts** (object): article, script, product
- **Events** (object): meeting
- **Logs** (object): correspondence
...
- **`is_blocked`** is a system-managed boolean attribute set by cascades when an entity has unresolved dependencies. It is orthogonal to the focus dimension — an entity can be `active` and blocked at the same time.
