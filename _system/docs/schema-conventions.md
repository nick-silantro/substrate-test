...
- `phase` — Where is this in its lifecycle? (concept, in_development, testing, live, mature, legacy, retired)

**FLAIR** (work-nature):
- `focus` — Am I working on this right now? (idle, active, waiting, blocked, paused, closed)
- `life_stage` — Where is this in its workflow? (backlog, ready, scheduled, in_progress, under_review, done_working)
...
```yaml
phase:
  values: [concept, in_development, testing, live, ...]
  type_defaults:
    friction: "live"      # observed frictions are real-world conditions
    agent: "live"         # runtime agents aren't concepts when created
    goal: "concept"       # goals are aspirational until they become plans
```
...
Example: `application_stage` on `job-opportunity` uses job-search industry vocabulary (saved/applied/screening/interviewing/offer/closed) that doesn't map cleanly to generic opportunity funnel stages. The type earns its own dimension.
