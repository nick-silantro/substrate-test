...
The key difference from a trace or a session log: a decision entity is **indexable and linkable**. It `governs` the entities it constrains. It `supersedes` earlier decisions it overrides. The graph can walk from a product entity to the decision that shaped it.
...
- **`decided_by` → [person/agent]** — Who made this call. Can reference a person entity or an agent entity. For session context, link to a note or trace capturing the session where it was decided.
